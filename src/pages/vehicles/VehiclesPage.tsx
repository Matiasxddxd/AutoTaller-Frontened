...
const VehicleModal = ({ open, onClose }: { open: boolean; onClose: () => void }) => {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Registrar vehículo" size="lg">
      <form onSubmit={handleSubmit(d => mutation.mutate(d))} className="space-y-4">
        ...
        <div className="flex gap-2 justify-end pt-1">
          ...
          <button type="submit" disabled={mutation.isPending || !selectedId} className="btn-primary">
            {mutation.isPending ? <><Spinner size={14} /> Guardando...</> : 'Registrar'}
          </button>
        </div>
      </form>
    </Modal>
...
export const VehiclesPage = () => {
  ...
  return (
    <div className="space-y-5">
              ...
              <tbody>
                {vehicles.map(v => (
                  <tr key={v.id} className="table-row cursor-pointer" onClick={() => navigate(`/vehicles/${v.id}`)}>
                    ...
                    <td className="td"><ChevronRight size={15} className="text-ink-faint" /></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <VehicleModal open={modal} onClose={() => setModal(false)} />
    </div>
  )
}
